...
static bool dln2_adc_event_timer_callback(adc_repeating_timer_t *rt) {
  LOG_INFO("%s\n", __func__);
  dln2_adc_event();
...
static bool dln2_adc_channel_set_cfg(struct dln2_slot *slot) {
  ...
  LOG_INFO(
      "DLN2_ADC_CHANNEL_SET_CFG: port=%u chan=%u type=%u period=%ums low=%u "
      "high=%u\n",
      cfg->port_chan.port, cfg->port_chan.chan, cfg->type, cfg->period,
      cfg->low, cfg->high);

  if (cfg->type != DLN2_ADC_EVENT_NONE && cfg->type != DLN2_ADC_EVENT_ALWAYS) {
    LOG_INFO("ADC event type not implemented\n");
   ...
   * It may occur, for example, if you set the zero event period for the ALWAYS
   * event type. For details, read Digital Input Events.
   */
  ...
  if (cfg->type == DLN2_ADC_EVENT_NONE && !cfg->period) {
    ...
    if (!_adc_driver->add_repeating_timer_us(-1000 * cfg->period,
                                             dln2_adc_event_timer_callback,
                                             NULL, dln2_adc_event_timer)) {
      LOG_INFO("ADC: Failed to add timer\n");
...
bool dln2_handle_adc(struct dln2_slot *slot) {
  ...
  case DLN2_ADC_SET_RESOLUTION:
    LOG_INFO("DLN2_ADC_SET_RESOLUTION\n");
